...
AHAND::AHAND()
{
    ...
    if (EatAsset.Succeeded()) { EatSoundAsset = EatAsset.Object; }

    static ConstructorHelpers::FObjectFinder<USoundBase> BGMAsset(TEXT("/Game/seisaku/Sound/onsen-ryokan-24.onsen-ryokan-24"));
    if (BGMAsset.Succeeded()) { BGMSoundAsset = BGMAsset.Object; }
}
...
void AHAND::BeginPlay()
{
    ...
    if (BGMAudioComponent && BGMSoundAsset)
    {
        BGMAudioComponent->SetSound(BGMSoundAsset);
    ...
    if (PC)
    {
        ...
        // エフェクトを強制的にオンにする
        PC->ConsoleCommand(TEXT("sg.PostProcessQuality 3"));
    }
...
void AHAND::MoveUp(float Value)
{
    ...
    if (Value != 0.0f && !bIsReadyToDisappear)
    {
        ...
        if (bIsGrabbing)
        {
            ...
            if (CurrentTime - LastScrollTime < 0.15f)
            {
            ...
            if (CurrentTime >= LastEatSoundTime + EatSoundCooldown)
            {
                if (EatSoundAsset)
                {
                    UGameplayStatics::PlaySound2D(this, EatSoundAsset);
